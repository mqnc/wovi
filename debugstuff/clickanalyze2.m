...
[~,h] = contourf(rand(nnspfield,nstepsfield));
...
nnspfield = size(get(h,'zdata'),1);
nstepsfield = size(get(h,'zdata'),2);
...
ll = linspace(cmin,0,1000);
ll(end)=[];
ll = [ll, linspace(0,cmax,20)];
...
c0 = zeros(90*k,3);
c1 = [zeros(10*k,1), linspace(1,0.2,10*k).', zeros(10*k,1)];
c2 = [linspace(0.2,1,100*k).', zeros(100*k,1), zeros(100*k,1)];
