...
FLOAT cost2(FLOAT8 joints, FLOAT8 jointsBefore){
	...
	//return -cos(joints.s0); // TEST REMOVE THIS
...
__kernel void ikray(
	...
	FLOAT delta = 0.5; // scout elbow parameter cw and ccw per step (in rad)
	...
	for(int ii=i*(*nsteps)*7; ii<(i+1)*(*nsteps)*7; ii++){
		debugOutput[ii] = 3;
	}

	// used for field coloring:
	...
	for(int rayStep = 1; rayStep <= *nsteps; rayStep++){ // step along the ray
		...
		if(ikres == false){outOfRange = true;}
		...
		if(cost2(jntsCurrent, jntsLastRayStep) > costCurrent){dir = -1.0;}
		...
		if(costLastElbowStep < 0){
			...
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 0] = jntsCurrent.s0;
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 1] = jntsCurrent.s1;
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 2] = jntsCurrent.s2;
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 3] = jntsCurrent.s3;
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 4] = jntsCurrent.s4;
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 5] = jntsCurrent.s5;
			debugOutput[i*(*nsteps)*7 + (rayStep-1)*7 + 6] = jntsCurrent.s6;
	...
	FLOAT3 fieldColor = (FLOAT3)(0,0.73,1);

	if(outOfRange){
		//fieldColor = (FLOAT3)(1.0,0.0,0.0);
	}

	rayColors[i].xyz = (qColor)*(FLOAT3)(0.9,0.9,0.9) + (1-qColor)*fieldColor;
